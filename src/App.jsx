...
function renderRichText(text) {
  ...
  const html = text
    ...
    .replace(/\*?\*?BODY\*?\*?:?\s*([\s\S]*?)(?=<div|\n\s*\*?\*?(?:HOOK|PATTERN|TRACCIA|BRIDGE|BODY|LOOP|GIORNO|VIDEO)\b|$)/gi, '<div style="background: rgba(var(--acc-green-rgb), 0.05); border: 1px solid rgba(var(--acc-green-rgb), 0.27); border-radius: 6px; padding: 10px; margin-bottom: 8px;"><div style="color: var(--acc-green); font-weight: 800; font-size: 11px; letter-spacing: 0.5px; text-transform: uppercase; margin-bottom: 4px;">🥩 BODY / PAYLOAD</div><div style="color: var(--text-main); font-size: 13px;">$1</div></div>')
