...
function renderRichText(text) {
  ...
  const html = text
    ...
    .replace(/(^|\n)([ \t]*)(1\.|2\.|3\.|4\.|5\.|6\.|7\.|8\.|9\.)\s*(.*?)(?=\n|$)/g, '$1$2<div style="display:flex;align-items:flex-start;margin-top:10px;margin-bottom:10px;"><div style="background: rgba(var(--acc-blue-rgb), 0.13); color: var(--acc-blue); width: 22px; height: 22px; text-align: center; border-radius: 50%; border: 1px solid rgba(var(--acc-blue-rgb), 0.27); font-weight: 800; font-size: 11px; line-height: 20px; margin-right: 10px; flex-shrink: 0; display:inline-block;">$3</div><div style="flex:1;">$4</div></div>')
    .replace(/^\s*[\-\*]\s+(.*?)(?=\n|$)/gm, '<div style="display:flex;align-items:flex-start;margin-top:6px;margin-bottom:2px;"><span style="color:var(--acc-green);margin-right:8px;font-weight:bold;flex-shrink:0;">•</span><span>$1</span></div>')
    .replace(/\*?\*?BODY\*?\*?:?\s*([\s\S]*?)(?=<div|\n\s*\*?\*?(?:HOOK|PATTERN|TRACCIA|BRIDGE|BODY|LOOP|GIORNO|VIDEO)\b|$)/gi, '<div style="background: rgba(var(--acc-green-rgb), 0.05); border: 1px solid rgba(var(--acc-green-rgb), 0.27); border-radius: 6px; padding: 10px; margin-bottom: 8px;"><div style="color: var(--acc-green); font-weight: 800; font-size: 11px; letter-spacing: 0.5px; text-transform: uppercase; margin-bottom: 4px;">🥩 CONTENUTO E SVOLGIMENTO</div><div style="color: var(--text-main); font-size: 13px;">$1</div></div>')
    .replace(/\*?\*?LOOP\*?\*?:?\s*([\s\S]*?)(?=<div|\n\s*\*?\*?(?:HOOK|PATTERN|TRACCIA|BRIDGE|BODY|LOOP|GIORNO|VIDEO)\b|$)/gi, '<div style="background: rgba(var(--acc-red-rgb), 0.05); border: 1px solid rgba(var(--acc-red-rgb), 0.27); border-radius: 6px; padding: 10px; margin-bottom: 12px;"><div style="color: var(--acc-red); font-weight: 800; font-size: 11px; letter-spacing: 0.5px; text-transform: uppercase; margin-bottom: 4px;">♻️ LOOP / CTA (FINE)</div><div style="color: var(--text-muted); font-size: 13px;">$1</div></div>')
